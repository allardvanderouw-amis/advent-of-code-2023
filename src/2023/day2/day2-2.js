...
const getGamePower = (gameLine) => {
  ...
  const sets = gameLine.split(':')[1].split(';')
  for (const set of sets) {
    const blocks = set.trim().split(',')

    for (const block of blocks) {
      let [amount, kind] = block.trim().split(' ')
      amount = Number(amount)
      if (kind === 'red') {
        red = red > amount ? red : amount
      }
      if (kind === 'green') {
        green = green > amount ? green : amount
      }
      if (kind === 'blue') {
  ...
  console.log({ power, red, green, blue })
