...
const run = () => {
  ...
  const [instruction, , ...elementLines] = lines
  const elementsMap = new Map()
  for (const elementLine of elementLines) {
    elementsMap.set(elementLine.substring(0, 3), [
      elementLine.substring(7, 10),
      elementLine.substring(12, 15),
    ])
  }


  const steps = []
  const keys = [...elementsMap.keys()]
  const starterKeys = keys.filter((key) => key[2] === 'A')
  for (const starterKey of starterKeys) {
    let stepCount = 0
    let instructionIndex = -1
    let element = elementsMap.get(starterKey)
    ...
    for (;;) {
      ...
      const lr = instruction[instructionIndex]
      let nextElementKey
      if (lr === 'L') {
        nextElementKey = element[0]
      } else {
        nextElementKey = element[1]
      }

      if (nextElementKey.endsWith('Z')) {
        break
      }
      element = elementsMap.get(nextElementKey)
    }
