...
const extractPartNumbers = (lines, gearIndex) => {
  ...
  let connected = []
  let numberString = ''
  let numberStringStart = null
  let numberStringEnd = null

  for (const [lineIndex, line] of lines.entries()) {
    for (let i = 0; i < line.length; i += 1) {
      const isNumber = line[i].match(pattern)
      ...
      if ((!isNumber && numberString) || (isNumber && numberString && isLast)) {
        // Number stopped
        numberStringEnd = i

        const top = lines[lineIndex - 1] ? lines[lineIndex - 1].substring(numberStringStart - 1, numberStringEnd + 1) : ''
        for (const [symbolIndex, symbol] of top.split('').entries()) {
          if (symbol === '*' && lineIndex === 2 && Math.max(numberStringStart - 1, 0) + symbolIndex === gearIndex) {
            connected.push(Number(numberString))
          }
        }

        const bottom = lines[lineIndex + 1] ? lines[lineIndex + 1].substring(numberStringStart - 1, numberStringEnd + 1) : ''
        for (const [symbolIndex, symbol] of bottom.split('').entries()) {
          if (symbol === '*' && lineIndex === 0 && Math.max(numberStringStart - 1, 0) + symbolIndex === gearIndex) {
            connected.push(Number(numberString))
          }
        }

        const left = line.substring(numberStringStart - 1, numberStringStart)
        for (const [symbolIndex, symbol] of left.split('').entries()) {
          if (symbol === '*' && lineIndex === 1 && Math.max(numberStringStart - 1, 0) + symbolIndex === gearIndex) {
            connected.push(Number(numberString))
          }
        }

        const right = line.substring(numberStringEnd, numberStringEnd + 1)
        for (const [symbolIndex, symbol] of right.split('').entries()) {
          if (symbol === '*' && lineIndex === 1 && numberStringEnd + symbolIndex === gearIndex) {
            connected.push(Number(numberString))
          }
        }
        ...
        // Reset number string
        numberString = ''
        numberStringStart = null
        numberStringEnd = null
      }
    }
  }
...
const extractGearNumber = (lines, lineIndex) => {
  let gearNumbersSum = 0
  ...
  for (let i = 0; i < line.length; i += 1) {
    const isGear = line[i] === '*'
    if (isGear) {
      const partNumbersSum = extractPartNumbers([
        lines[lineIndex - 1] || '',
        lines[lineIndex] || '',
        lines[lineIndex + 1] || '',
      ], i)
      gearNumbersSum += partNumbersSum
    }
  }

  // console.log(`${line}: ${gearNumbersSum}`)
...
const run = () => {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    const gearNumbersSum = extractGearNumber(lines, lineIndex)

    sum = sum + gearNumbersSum
  }
