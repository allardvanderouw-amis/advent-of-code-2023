...
const extractPartNumbers = (lines, lineIndex) => {
  ...
  for (let i = 0; i < line.length; i += 1) {
    ...
    if ((!isNumber && numberString) || (isNumber && numberString && isLast)) {
      // Number stopped
      numberStringEnd = i
      // console.log({ numberString, numberStringStart, numberStringEnd })
      ...
      const round = [top, bottom, left, right].join('')
      const symbols = round.split('')
      const allDots = round.split('').every((value) => value === '.' || value.match(pattern))
      console.log({ numberString, round, valid: !allDots })
      if (!allDots) {
        partNumbersSum += Number(numberString)
      }
  ...
  // console.log(`${line}: ${partNumbersSum}`)
...
const run = () => {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    const partNumbersSum = extractPartNumbers(lines, lineIndex)

    sum = sum + partNumbersSum
  }
