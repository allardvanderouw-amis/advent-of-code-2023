...
const getLoopPositions = () => {
  let sum = 0
  const grid = input2.trim().split('\n').map((line) => line.split(''))

  // Get starting point
  const startingPoint = grid.reduce((acc, gridLine, gridLineIndex) => {
    for (const [gridItemIndex, gridItem] of gridLine.entries()) {
      if (gridItem === 'S') {
        acc.x = gridItemIndex
        acc.y = gridLineIndex
      }
  ...
  // Get connected grid items to start
  const connectedGridItems = []
  if (grid[startingPoint.y - 1] && grid[startingPoint.y - 1][startingPoint.x] !== '.') {
    // Top is connected
    connectedGridItems.push({ x: startingPoint.x, y: startingPoint.y - 1 })
  }
  if (grid[startingPoint.y] && grid[startingPoint.y][startingPoint.x + 1] !== '.') {
    // Right is connected
    connectedGridItems.push({ x: startingPoint.x + 1, y: startingPoint.y })
  }
  if (grid[startingPoint.y + 1] && grid[startingPoint.y + 1][startingPoint.x] !== '.') {
    // Bottom is connected
    connectedGridItems.push({ x: startingPoint.x, y: startingPoint.y + 1 })
  ...
  console.log('connectedGridItems', connectedGridItems)

  // Follow items
  const routes = []
  for (let connectedGridItem of connectedGridItems) {
    console.log(`Starting at ${grid[connectedGridItem.y][connectedGridItem.x]}`)

    let steps = 1
    let prevGridItem = { ...startingPoint }
    let gridItem = { ...connectedGridItem }
    let gridItems = []
    while (true) {
      gridItems.push(gridItem)
      if (!grid[gridItem.y] || !grid[gridItem.y][gridItem.x]) {
        break
      }
      if (grid[gridItem.y][gridItem.x] === '.') {
        break
      }
      if (grid[gridItem.y][gridItem.x] === 'S') {
        // Route completed
        routes.push({
          connectedGridItem,
          steps,
      ...
      switch (grid[gridItem.y][gridItem.x]) {
        case '|':
          newGridItem.y = prevGridItem.y < gridItem.y ? gridItem.y + 1 : gridItem.y - 1
          break;
        case '-':
          newGridItem.x = prevGridItem.x < gridItem.x ? gridItem.x + 1 : gridItem.x - 1
          break;
        case 'L':
          newGridItem.x = prevGridItem.x === gridItem.x ? gridItem.x + 1 : gridItem.x
          newGridItem.y = prevGridItem.y === gridItem.y ? gridItem.y - 1 : gridItem.y
          break;
        case 'J':
          newGridItem.x = prevGridItem.x === gridItem.x ? gridItem.x - 1 : gridItem.x
          newGridItem.y = prevGridItem.y === gridItem.y ? gridItem.y - 1 : gridItem.y
        ...
        case 'F':
          ...
          break;
        default:
          throw new Error(`Invalid grid item: ${grid[gridItem.y][gridItem.x]}`)
      }
  ...
  const largestRoute = routes.sort((a, b) => b.steps - a.steps)[0]

  return largestRoute
}

const run = async () => {
  const largestRoute = getLoopPositions()
  console.dir(largestRoute, { depth: null })

  let input = input2

  let grid = input.trim().split('\n').map((line) => line.split(''))
  for (const [gridLineIndex, gridLine] of grid.entries()) {
    for (const [gridItemIndex, gridLineItem] of gridLine.entries()) {
      const isPath = largestRoute.gridItems.some((item) => {
        if (item.y === gridLineIndex && item.x === gridItemIndex) {
          return true
        }
        return false
      })
      ...
      grid[gridLineIndex][gridItemIndex] = '.'
    }
  }
  ...
  for (const [gridLineIndex, gridLine] of grid.entries()) {
    ...
    enhancedGrid.push([])
    enhancedGrid.push([])
    for (const gridLineItem of gridLine) {
      if (gridLineItem === '.') {
        enhancedGrid[((gridLineIndex + 1) * 3) - 3].push('.', '.', '.')
        enhancedGrid[((gridLineIndex + 1) * 3) - 2].push('.', '.', '.')
        enhancedGrid[((gridLineIndex + 1) * 3) - 1].push('.', '.', '.')
      }
      if (gridLineItem === 'S') {
        enhancedGrid[((gridLineIndex + 1) * 3) - 3].push('.', 'X', '.')
        enhancedGrid[((gridLineIndex + 1) * 3) - 2].push('X', 'X', 'X')
        enhancedGrid[((gridLineIndex + 1) * 3) - 1].push('.', 'X', '.')
      }
      if (gridLineItem === 'F') {
        enhancedGrid[((gridLineIndex + 1) * 3) - 3].push('.', '.', '.')
        enhancedGrid[((gridLineIndex + 1) * 3) - 2].push('.', 'X', 'X')
        enhancedGrid[((gridLineIndex + 1) * 3) - 1].push('.', 'X', '.')
      }
      if (gridLineItem === '7') {
        enhancedGrid[((gridLineIndex + 1) * 3) - 3].push('.', '.', '.')
      ...
      if (gridLineItem === '|') {
        enhancedGrid[((gridLineIndex + 1) * 3) - 3].push('.', 'X', '.')
        enhancedGrid[((gridLineIndex + 1) * 3) - 2].push('.', 'X', '.')
        enhancedGrid[((gridLineIndex + 1) * 3) - 1].push('.', 'X', '.')
      }
    }
  }
  console.log(enhancedGrid)

  input = enhancedGrid.map((gridLine) => gridLine.join('')).join('\n')

  input = input.replaceAll('.', 0)
  input = input.replaceAll('X', 1)
  console.log(input)
  ...
  let routesFound = 0
  let routesNotFound = 0

  for (const [gridLineIndex, gridLine] of grid.entries()) {
    if (gridLineIndex % 3 !== 1) {
      continue
    }

    for (const [gridItemIndex, gridItem] of gridLine.entries()) {
      if (gridItemIndex % 3 !== 1) {
        continue
      }
      if (gridItem === 1) {
        continue
      ...
      let path = null
      easystar.findPath(gridItemIndex, gridLineIndex, 0, 0, (p) => {
        path = p
      })
      easystar.calculate()
  ...
  console.dir({ routesFound, routesNotFound })
}
run()